				...
				<div id="home_about">
					...
					<div
						...
						<table align="center" border="1" cellspacing="1" cellpadding="1"
							...
							PreparedStatement ps2 = con.prepareStatement("DELETE FROM `instance`");
							
							//ResultSet rs1 = ps1.executeQuery();
						int rs2 =  ps2.executeUpdate();
						
							PreparedStatement ps = con
										.prepareStatement("SELECT DISTINCT a, b, c, d, e, f, g, h, i, j, k, l, m, n, o, p, q, r, s, t, u, v, w, x, y, z, aa, bb, cc, dd, ee, ff, gg, hh, ii, jj, kk,ll,mm,nn,oo,pp,qq FROM nsl_kdd_3000");
							...
							<tr>
								...
								<td><%=rs.getString("bb")%></td>
								<td><%=rs.getString("cc")%></td>
								<td><%=rs.getString("dd")%></td>
								<td><%=rs.getString("ee")%></td>
								<td><%=rs.getString("ff")%></td>
								<td><%=rs.getString("gg")%></td>
								<td><%=rs.getString("hh")%></td>
								<td><%=rs.getString("ii")%></td>
								<td><%=rs.getString("jj")%></td>
								<td><%=rs.getString("kk")%></td>
								<td><%=rs.getString("ll")%></td>
								<td><%=rs.getString("mm")%></td>
								<td><%=rs.getString("nn")%></td>
								<td><%=rs.getString("oo")%></td>
								<td><%=rs.getString("pp")%></td>
								...
								<%
									a = rs.getString("a");
										...
										PreparedStatement ps1 = con
												.prepareStatement("INSERT INTO `instance`(`a`, `b`, `c`, `d`, `e`, `f`, `g`, `h`, `i`, `j`, `k`, `l`, `m`, `n`, `o`, `p`, `q`, `r`, `s`, `t`, `u`, `v`, `w`, `x`, `y`, `z`, `aa`, `bb`, `cc`, `dd`, `ee`, `ff`, `gg`, `hh`, `ii`, `jj`, `kk`, `ll`, `mm`, `nn`, `oo`, `pp`, `qq`) VALUES ('"+a+"','"+b+"','"+c+"','"+d+"','"+e+"','"+f+"','"+g+"','"+h+"','"+i+"','"+j+"','"+k+"','"+l+"','"+m+"','"+n+"','"+o+"','"+p+"','"+q+"','"+r+"','"+s+"','"+t+"','"+u+"','"+v+"','"+w+"','"+x+"','"+y+"','"+z+"','"+aa+"','"+ab+"','"+ac+"','"+ad+"','"+ae+"','"+af+"','"+ag+"','"+ah+"','"+ai+"','"+aj+"','"+ak+"','"+al+"','"+am+"','"+an+"','"+ao+"','"+ap+"','"+aq+"')");
